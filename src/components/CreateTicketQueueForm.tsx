...
const CreateTicketQueueForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isMedicQueue && !medicId) {
      setMessage({ type: 'error', text: 'Please select a medic for a medic queue.' });
      return;
    }
    ...
    const payload: TicketQueueRequestDTO = {
      ...
      consultationRoom: (isMedicQueue && consultationRoom) ? Number(consultationRoom) : undefined
    };

    try {
      await ApiService.createTicketQueue(payload);
      setMessage({ type: 'success', text: 'Ticket Queue created successfully!' });
      setConsultationRoom('');
      setMedicId('');
    } catch (err: any) {
      ...
      } else if (errorMsg.includes('409')) { // Fallback for status code check if message is generic
         if (isMedicQueue) {
          errorMsg = 'A queue for this medic already exists today.';
        } else {
          errorMsg = 'A general queue already exists today.';
    ...
    <div className="p-6">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="flex items-center mb-4">
          ...
          />
          <label htmlFor="medicQueue" className="ml-2 block text-sm text-gray-900">
            Queue for a medic?
          </label>
        </div>
        ...
        {isMedicQueue && (
          <>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="medicId">
                Medic
              </label>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setConsultationRoom(e.target.value ? Number(e.target.value) : '')}
                className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2.5 border bg-white"
                placeholder="e.g. 101"
                required={isMedicQueue}
              />
