...
const MedicList: React.FC = () => {
  ...
  // Edit State
  const [editingMedic, setEditingMedic] = useState<MedicResponseDTO | null>(null);
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Are you sure you want to delete this medic? This action cannot be undone.')) return;
    try {
  ...
  if (loading && medics.length === 0) return <div className="p-6 text-center text-gray-500">Loading directory...</div>;
    ...
    <div className="overflow-x-auto relative">
      <table className="min-w-full divide-y divide-gray-200">
        ...
        <tbody className="bg-white divide-y divide-gray-200">
            ...
            <tr key={medic.id} className="hover:bg-gray-50">
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <div className="text-sm font-medium text-gray-900">{medic.person?.name}</div>
                  </div>
                </div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-mono">{medic.crm}</td>
              <td className="px-6 py-4 whitespace-nowrap">
          ...
          {medics.length === 0 && (
            <tr>
              <td colSpan={4} className="px-6 py-8 text-center text-gray-500">No medics found.</td>
            </tr>
          )}
        </tbody>
      </table>
          ...
          <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-md">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input type="text" value={editCrm} onChange={e => setEditCrm(e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm border p-2" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Specialty</label>
                <input type="text" value={editType} onChange={e => setEditType(e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm border p-2" />
              </div>
            </div>

            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setEditingMedic(null)} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">Cancel</button>
