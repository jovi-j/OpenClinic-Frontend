...
const AdminDashboard: React.FC<{ onExit: () => void }> = ({ onExit }) => {
  const [activeTab, setActiveTab] = useState<'PATIENTS' | 'MEDICS' | 'ATTENDANTS'>('PATIENTS');
    ...
    <div className="min-h-screen bg-gray-100">
      <header className="bg-gray-800 text-white p-4 shadow-md sticky top-0 z-50">
        <div className="container mx-auto flex justify-between items-center">
          ...
          <button onClick={onExit} className="text-sm bg-gray-700 hover:bg-gray-600 px-3 py-1 rounded">Exit Admin</button>
        </div>
      </header>
