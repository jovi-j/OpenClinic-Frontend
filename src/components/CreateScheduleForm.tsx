...
const CreateScheduleForm: React.FC<CreateScheduleFormProps> = ({ medicId: propMedicId }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'Schedule created successfully! Appointments generated.' });
    } catch (err: any) {
  ...
  return (
    <div className="p-6">
      <form onSubmit={handleSubmit} className="space-y-4">
        {!propMedicId && (
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Medic</label>
            <select
              value={medicId}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Attendance Start</label>
            <input
              type="time"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Attendance End</label>
            <input
              type="time"
              value={attendanceEnd}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Avg Appointment Time (min)</label>
          <input
            type="number"
            min="5"
            step="5"
