...
import { MedicRequestDTO } from '../types/api';
...
const RegisterMedicForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await ApiService.createMedic(payload);
      setMessage({ type: 'success', text: 'Medic registered successfully!' });
      setName('');
      setCpf('');
      setDob('');
      setCrm('');
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
      <h3 className="text-lg font-semibold text-gray-800 mb-4">Register New Medic</h3>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input type="text" value={crm} onChange={e => setCrm(e.target.value)} required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm border p-2" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700">Specialty (Type)</label>
          <input type="text" value={type} onChange={e => setType(e.target.value)} required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm border p-2" />
        </div>
        <button type="submit" disabled={loading} className="w-full bg-purple-600 text-white py-2 px-4 rounded-md hover:bg-purple-700 disabled:opacity-50">
          {loading ? 'Registering...' : 'Register Medic'}
        </button>
      </form>
      {message && (
