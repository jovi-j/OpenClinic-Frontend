...
import { TicketResponseDTO, TicketQueueResponseDTO, AttendantResponseDTO, PatientResponseDTO } from '../types/api';
...
const TicketDisplay: React.FC<{ onExit: () => void }> = ({ onExit }) => {
  ...
  const fetchData = async () => {
    try {
      const [ticketsData, queuesData, attendantsData, patientsData] = await Promise.all([
  ...
  const calledTickets = tickets
    .filter(t => {
      ...
      return t.status === 'CALLED_BY_ATTENDANT' || t.status === 'CALLED_BY_MEDIC';
    })
    .reverse();
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8 flex flex-col">
      <div className="flex justify-between items-center mb-8">
        ...
        <button onClick={onExit} className="text-gray-600 hover:text-gray-400">Exit</button>
      </div>
          ...
          {currentTicket ? (
            <>
              <div className="absolute top-0 left-0 w-full bg-blue-700 py-4 text-center">
                <h2 className="text-4xl font-bold uppercase tracking-widest text-blue-100">Now Calling</h2>
              </div>
              ...
              <div className="text-5xl font-bold mb-12 text-blue-200">
                {currentTicket.ticketPriority === 'NMT' ? 'Normal' : 
                 currentTicket.ticketPriority === 'PRT' ? 'Preferential' : 'Exam Results'}
              </div>

              <div className="bg-white text-blue-900 px-12 py-6 rounded-2xl shadow-lg">
                <div className="text-2xl font-medium uppercase tracking-wide mb-2 text-center text-gray-500">Please proceed to</div>
            ...
            <div className="text-4xl text-blue-300 font-light">Waiting for tickets...</div>
          )}
        </div>

        {/* History (Right/Sidebar) */}
        <div className="w-1/3 flex flex-col gap-4">
          <h3 className="text-2xl font-semibold text-gray-400 mb-2 px-2">Last Called</h3>
          {historyTickets.map(ticket => (
            <div key={ticket.id} className="bg-gray-800 rounded-xl p-6 flex justify-between items-center border border-gray-700 shadow-lg opacity-80">
              <div>
                <div className="text-4xl font-bold text-white">#{ticket.ticketNum}</div>
                <div className="text-sm text-gray-400 mt-1">
                  {ticket.ticketPriority === 'NMT' ? 'Normal' : 
                   ticket.ticketPriority === 'PRT' ? 'Preferential' : 'Exam Results'}
                </div>
